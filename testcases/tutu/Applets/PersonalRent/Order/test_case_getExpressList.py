...
from interface.tutu.PersonalRent.Order.getExpressList import getExpressList
...
class TestPersonalRentFunc(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ex = excelUtil()
        self.ListQuery = getExpressList()

    def test_getExpressList_tutu_Android_001(self):
        ...
        #获取快递公司列表接口请求URL
        ListQueryURL = self.ListQuery.get_getExpressListURL(self.config.get('imi_base_url_ch', 'base_url_uat'),
                                                  self.config.get("PersonalRent", "getExpressListURL"), self.config.get("lang", "zh"),
                                                  self.base.getTimeStamp(),
                                                  self.config.get("clientVersionInfo", "clientVersionInfo_ch_Android"),access_token)

        result_ListQuery = self.ListQuery.send_request_getExpressListURL(ListQueryURL)
